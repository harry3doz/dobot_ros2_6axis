...
#include "dobot_msgs/action/mov_j.hpp>
...
#include <dobot_msgs/srv/acc_l.hpp>
...
#include <dobot_msgs/msg/robot_status.hpp>
...
#include <dobot_msgs/srv/stop_move_jog.hpp>
...
private:
    ...
    rclcpp::TimerBase::SharedPtr backend_task_timer_;
    double trajectory_duration_;
    std::shared_ptr<rclcpp::Node> node_;
    rclcpp_action::Server<dobot_msgs::action::MovJ>::SharedPtr action_server_;
    //rclcpp::Client<dobot_msgs::srv::MovJ>::SharedPtr cli_movj;
    std::shared_ptr<CR5Commander> commander_;
    std::vector<rclcpp::ServiceBase::SharedPtr> server_tbl_;
    uint16_t last_robot_mode_;
    std::thread thread_pub_feedback_info;
    rclcpp::Publisher<ServoJParam>::SharedPtr k_servoj_param;
    
    static int str2Int(const char* val);

    rclcpp_action::GoalResponse handle_goal(const rclcpp_action::GoalUUID & uuid, 
                                                std::shared_ptr<const dobot_msgs::action::MovJ::Goal> goal);
    rclcpp_action::CancelResponse  handle_cancel(const std::shared_ptr<rclcpp_action::ServerGoalHandle<dobot_msgs::action::MovJ>> goal_handle);
    void handle_accepted(const std::shared_ptr<rclcpp_action::ServerGoalHandle<dobot_msgs::action::MovJ>> handle);
    void execute(const std::shared_ptr<rclcpp_action::ServerGoalHandle<dobot_msgs::action::MovJ>> handle);
    //void backendTask(const rclcpp::TimerEvent &e);

    //void feedbackHandle(const std::shared_ptr<rclcpp_action::ServerGoalHandle<control_msgs::action::FollowJointTrajectory>> handle);
    //void moveHandle(const std::shared_ptr<rclcpp_action::ServerGoalHandle<control_msgs::action::FollowJointTrajectory>> handle);
    //void handle_goal(std::shared_ptr<rclcpp_action::ServerGoalHandle<control_msgs::action::FollowJointTrajectory>> handle);
    //void handle_cancel(std::shared_ptr<rclcpp_action::ServerGoalHandle<control_msgs::action::FollowJointTrajectory>> handle);
    //void backendTask(const rclcpp::TimerEvent &e);
    //void pubFeedBackInfo();
    std::vector<std::string> regexRecv(std::string get_recv_info)


public:

    CR5Robot(std::shared_ptr<rclcpp::Node> nh, std::string name);
    DOBOT_BRINGUP_PUBLIC
    explicit CR5Robot(std::shared_ptr<rclcpp::Node> nh, std::string name);
    ~CR5Robot() override;
    void init();
...
protected:
    ...
    bool continues(
            const std::shared_ptr<dobot_msgs::srv::Continues::Request> request,
            std::shared_ptr<dobot_msgs::srv::Continues::Response> response);
...
//     template <typename T>
//     template <typename T>
//     T holdRegs(int &index, int &addr, const int count, const std::string type){
//         T srv;
...
//     template <typename T>
//     T holdRegs(int &index, int &addr, const int count, const std::string regs, const std::string type){
//         T srv;
//         srv.request.index = index;
//         srv.request.addr = addr;
//         srv.request.count = count;
//         srv.request.regs = regs;
//         srv.request.type = type;
//         return srv;
//     }
...
//     template <typename T>
//     T passJoints(Coordinate& cod){
//         T srv;
//         srv.request.j1 = cod.getX();
//         srv.request.j2 = cod.getY();
//         srv.request.j3 = cod.getZ();
//         srv.request.j4 = cod.getRx();
//         srv.request.j5 = cod.getRy();
//         srv.request.j6 = cod.getRz();
//         return srv;
...
//     template <typename T>
//     T passCoordinate(Coordinate& cod){
//         T srv;
//         srv.request.x = cod.getX();
//         srv.request.y = cod.getY();
//         srv.request.z = cod.getZ();
//         srv.request.a = cod.getRx();
//         srv.request.b = cod.getRy();
//         srv.request.c = cod.getRz();
//         return srv;
//     }

};

} // end namespace cr5_robot

RCLCPP_COMPONENTS_REGISTER_NODE(dobot_bringup::CR5Robot)